<?php
	if(ISSET($_REQUEST["t2tFName"])) {
        $t2tFName = stripslashes(trim(str_replace("'", "", $_REQUEST["t2tFName"])));
        $isValid = array('-', '.', '/', ' ', '\'');
        if (ctype_alpha(str_replace($isValid, '', $t2tFName)) && (strlen($t2tFName) >= 2)  && (strlen($t2tFName) <= 25)) {
            echo "true";
        } else {
            echo "false";
        }
