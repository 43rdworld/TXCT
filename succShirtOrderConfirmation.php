...
//ECHO "PageName: ".$_SERVER['HTTP_REFERER']."<br>";
        if(truncatePage(strtolower(returnPageName($_SERVER['HTTP_REFERER']))) == 'succshirtorder.php') {
            //echo "SUBMIT COMING FROM COOKIE SQUAD HOMEPAGE<br>";
            if (strtolower($_POST["submitRegistration"]) == 'submitsuccshirtorder') {
                ...
                if (returnRegistrationStatus ($dbh,'EXEC sp_GetUserID :tableName,:formSecret','id','tbl_TCT_CookieSquadShirtSales',$_POST['formSecret']) == '') {
                    setVars('formSecret:str,serviceUnit:int,orderS:int,orderM:int,orderL:int,orderXL:int,order2X:int,order3X:int,orderTotal:int,orderDelivery:str,orderFName:caps,orderLName:caps,orderAddress:caps,orderCity:caps,orderState:str,orderZip:int,orderPhone:phn,orderEmail:str,billingSame:bit,billingFName:caps,billingLName:caps,billingAddress:caps,billingCity:caps,billingState:str,billingZip:int,billingPhone:phn,billingEmail:str,ccNum:str,ccExpMonth:str,ccExpYear:str,ccCVV2:str,ccType:str,orderItemized:str,orderSubTotalTemp:mon,orderTaxTemp:mon,orderGrandTotalTemp:mon', 0, 1, 'SU Cookie Coordinator TShirt Order Form');
